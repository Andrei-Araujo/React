...
export const cursosReducer = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    case TYPE_CURSOS_GET_LISTA:
      return { ...state, lista: action.value };
    case TYPE_CURSOS_SET_CODIGO:
      return {
        ...state,
        codigo: action.value,
        isCodigoValido: action.isValido,
      };
    case TYPE_CURSOS_SET_DESCRICAO:
      return {
        ...state,
        descricao: action.value,
        isDescricaoValido: action.isValido,
      };
    case TYPE_CURSOS_SET_CARGA_HORARIA:
      return {
        ...state,
        cargaHoraria: action.value,
        isCargaHorariaValido: action.isValido,
      };
    case TYPE_CURSOS_SET_PRECO:
      return { ...state, preco: action.value, isPrecoValido: action.isValido };
    case TYPE_CURSOS_SET_CATEGORIA:
      return {
        ...state,
        categoria: action.value,
        isCategoriaValido: action.isValido,
      };
    case TYPE_CURSOS_LIMPAR_FORM:
      return { ...INITIAL_STATE, lista: state.lista };
    case TYPE_CURSOS_SELECIONAR_FORM:
      return {
        ...state,
