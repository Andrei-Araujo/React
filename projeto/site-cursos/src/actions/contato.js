...
export const setContatoEmail = (e) => {
  return (dispatch) => {
    ...
    if (
      ...
      dispatch(
        setContatoMsgErro("Favor preencher o campo email com um email válido.")
      );
    } else {
  ...
  return async (dispatch) => {
    try {
      ...
      if (!assunto || assunto.length < 5) {
        erros +=
          "Favor preencher o campo assunto com pelo menos cinco caracteres.";
      }
      ...
      dispatch(
        setContatoMsgErro("Não foi possível enviar contato, tente novamente!")
      );
      console.log(e);
    }
  };
